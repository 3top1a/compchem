...
export const createRecord = async (community, metadata) => {
  ...
  const response = await safeFetch(`${API_URL}/experiments`, {
    method: "POST",
    headers: {
...
export const editRecord = async (recordId, metadata) => {
  ...
  const response = await safeFetch(`${API_URL}/experiments/${recordId}/draft`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
    },
...
export const listFilesOfRecord = async (recordId) => {
  return await safeFetch(`${API_URL}/experiments/${recordId}/draft/files`, {
    method: "GET",
  });
};
...
export const uploadFileToRecord = async (recordId, file) => {
  ...
  if (!commitResponse.ok) deleteFileFromRecord(recordId, remoteFileName);
...
export const downloadFileFromRecord = async (
  recordId,
  remoteFileName,
  isDraft,
) => {
